 ...
 * @param {string} [options.outDir] Output directory, resolved relative to cwd.
 */
function runExport(cwd = process.cwd(), options = {}) {
  ...
  fs.mkdirSync(getStorageDir(outputDir), { recursive: true });
  ...
  const buildLogPath = path.join(outputDir, 'BUILD_LOG.md');
  ...
  const contextMdPath = path.join(getStorageDir(outputDir), 'CONTEXT.md');
